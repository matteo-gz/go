...
//
// 一个 map 就是一个哈希表。数据被组织成一个桶（bucket）的数组。
// 每个桶最多包含 8 个 key/elem 对，哈希的低位用于选择桶，
// 每个桶包含一些哈希的高位，用于区分单个桶内的条目。
//
// 如果多于 8 个键散列到同一个桶中，我们会在额外的桶上形成链表。
//
// 当哈希表增长时，我们会分配一个两倍于原有桶数的新的桶数组。
// 桶逐步从旧的桶数组中复制到新的桶数组中。
//
// Map 迭代器遍历桶数组，按照遍历顺序返回键（按照桶号、溢出链表顺序、桶内索引）。
// 为了维护迭代语义，我们从不移动键在其桶内的位置（如果移动，则可能返回 0 或 2 次键）。
...
//
// %overflow = 有溢出桶的桶的百分比
// bytes/entry = 每个键值对使用的额外字节数
// hitprobe = 查找存在的键时要检查的条目数量
// missprobe = 查找不存在的键时要检查的条目数量
//
// 请记住，这些数据是针对最大负载的表格，即表格增长之前。典型的表格将会有更少的负载。
